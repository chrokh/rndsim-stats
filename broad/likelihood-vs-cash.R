...
prepare <- function(df) {
  ...
  df$cash <- roundToNearest(df$proj_tot_cash, binSize)

  print('Counting entries per group')
  df <- ddply(df,
              c('intervention', 'cash', 'interventions_tot_size', 'grants_tot_size'),
              summarise,
...
findMeans <- function(df) {
  df <- ddply(df, c('cash', 'interventions_tot_size'), summarise,
...
doPlot <- function(df, name) {
  ...
  plot(
       ...
       col  = 'white',
       pch  = 19,
       xlab = paste('Total Projected Project Revenue ( \u00B1', binSize / 2, ')'),
...
quartile <- max(df$cash) / 4
df <- subset(df, df$cash < quartile * 3)
df <- subset(df, df$cash > min(df$cash))
...
mtext('Likelihood of Entry vs Projected Project Revenue', outer=TRUE,  cex=1, line=-2)
