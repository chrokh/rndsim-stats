...
prepare <- function(df) {
  ...
  print('Remove rows where discovery rate < 1 since they contain so little data')
  df <- subset(df, df$proj_type_discovery_rate >= 1)

  print('Round discovery rate')
  df$rate <- roundToNearest(df$proj_type_discovery_rate, binSize)
