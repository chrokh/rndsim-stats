...
plot1 <- function(df) {
  ...
  boxcolors  <- rep(colorRampPalette(colors)(nTypes), nTypes)
...
# ============================================
plot2 <- function(df) {
  df <- ddply(df, c('intervention', 'intervention_size'), summarise,
              pois = sum(pois),
              projs = sum(projs))
  ...
  # Define tick marks
  yticks <- seq(roundToNearest(min(df$pois), 0.25), roundToNearest(max(df$pois), 0.25), by = 0.25)
  ...
  for (intr in interventions) {
    sub <- subset(df, df$intervention == intr)
...
plot3 <- function(df) {
  ...
  # Define tick marks
  yticks <- seq(roundToNearest(min(df$delta), 5), round(max(df$delta), 1), by = 5)
  xticks <- sort(unique(df$intervention_size))
...
plot4 <- function(df, heatMapColors, name) {
  ...
  for (x in xs) {
    ...
    for (y in ys) {
      ...
      # Compute numbers to print on cells
      percentage = round(likelihood * 100, 2)
      totalSpend = round(pullSpend + pushSpend, 1)
      pullSpend  = round(pullSpend, 1)
      pushSpend  = round(pushSpend, 1)

      y1 = y + (1 / length(ys) / 3)
      y2 = y
      y3 = y + (- 1 / length(ys) / 3)

      text(x, y1, pushSpend, cex = small)
      text(x, y2, percentage, cex = large, font = 2)
      text(x, y3, pullSpend, cex = small)
  ...
  # y axis
  sizes  <- sort(unique(df$grants * 100))
  nSizes <- length(sizes)
  axis(2,
  ...
  mtext('Total push size for expected cost', side=4, line=3, cex=0.7)
}
...
plotToFile(makePath('boxplot-likelihood'))
plot1(df)

plotToSmallLandscape(makePath('lineplot-likelihood'))
plot2(df)

plotToSmallLandscape(makePath('lineplot-delta'))
plot3(df)

plotToSmallLandscape(makePath('levelplot-fd'))
layout(matrix(c(1,1,2,2,2,2,2,2,2,2,2,2,3), nrow = 1, byrow = TRUE))
plot4(subset(df, df$intervention == 'FDMER'), fdHeatMapColors, 'Full Delinkage')

plotToSmallLandscape(makePath('levelplot-pd'))
layout(matrix(c(1,1,2,2,2,2,2,2,2,2,2,2,3), nrow = 1, byrow = TRUE))
