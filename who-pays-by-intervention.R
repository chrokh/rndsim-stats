...
prepare <- function(df) {
  ...
  prepareIntervention <- function(df, intervention) {

    print(paste('Preparing subset with intervention size', intervention))
    df <- subset(df, df$interventions_tot_size == intervention)
  ...
  # Prepare by interventions
  interventions <- unique(df$interventions_tot_size)
  all <- data.frame()
  for (intervention in interventions) {
    single <- prepareIntervention(df, intervention)
...
if (args$only_entries) {
  cache <- 'who-pays-for-entries-by-intervention.csv'
} else {
  cache <- 'who-pays-by-intervention.csv'
}
...
if (args$only_entries) {
  title <- paste('Who pays for projects (that reach market), per intervention?', args$label)
} else {
  title <- paste('Who pays for projects, per intervention?', args$label)
}
boxplot(df$amount ~ interaction(df$spender, df$intervention),
        boxfill = boxcolors,
        las = 2,
        main = title,
...
group_positions <-
  seq(
      from = 4 + 0.5,
      to = (length(unique(df$intervention)) * 4),
      by = 4
      )
abline(h = NULL, v = group_positions, col = 'black', lty = 'solid')
