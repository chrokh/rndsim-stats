...
findMeans <- function(df) {
  df <- ddply(df, c('cash', 'intervention_size'), summarise,
...
doPlot <- function(df, context, name) {
  ...
  plot(
       df$mean_pois ~ df$cash,
       #col  = df$color,
       col  = 'white',
       pch  = 19,
       yaxt = 'n',
       xaxt = 'n',
       xlab = paste('Total Projected Antibiotic Revenue ( \u00B1', revenueBinSize / 2, ')'),
       ylab = 'Mean Likelihood of Market Entry (%)',
       ylim = c(min(context$mean_pois), max(context$mean_pois))
       )

  xticks <- seq(round(min(context$cash)), round(max(context$cash)), by = 500)
  ...
  for (bin in interventions) {
    ...
    lines(sub$mean_pois ~ sub$cash, col = sub$color)
  }
...
scenario <- function(df) {
  return(subset(df, df$thresh >= 200 &
         df$thresh <= 500 &
         df$rate >= 20))
}

df <- subset(df, df$cash > 0 & df$cash <= 6000)
...
doPlot(sub3, all, 'FD (BP Threshold 200-500 + VC Discount Rate 20-30%)')
doPlot(sub4, all, 'PD (BP Threshold 200-500 + VC Discount Rate 20-30%)')

mtext('Likelihood of Market Entry vs Projected Antibiotic Revenue', outer=TRUE,  cex=1, line=-2, font=2)
mtext('Likelihood of Market Entry vs Projected Antibiotic Revenue For Suggested Scenario', outer=TRUE,  cex=1, line=-28.7, font=2)
